...
class ReviewList(Resource):
    ...
    def post(self):
        ...
        for review in reviews: #El usuario ya hizo una review del place
            if review.place_id == review_data['place_id'] and review.user_id == review_data['user_id']:
                return {'error': "You have already reviewed this place."}, 400

        if user is None: #Usuario no existe
            return {"error": "User not found"}, 400

        if place is None: #Place no existe
            return {"error": "place not found"}, 400
        
        if review_data['user_id'] == place.owner_id: #El owner no puede postear una review
